...
struct CSVConfig {
    ...
    char delimiter = ',';
    // empty/invalid cells produce NaN.
    // Zero-fill silently corrupted the feature distribution because 0 is a
    // valid sensor reading (resting accelerometer axis, unused channel, etc).
    // NaN propagates cleanly through FeatureProcessor::compute_stats and
    // COCAModel::compute_losses (both skip NaN terms).
...
private:
    ...
    // stats count NaN (missing) rather than zero.
    // When default_value was 0.0f, all-zero and all-missing were
    // indistinguishable. Now they're separate: nan_counts flags genuine
    // missingness, min/max ignore NaN.
    void print_stats(const std::vector<std::vector<float>>& data) {
        ...
        if (config.has_timestamp) std::cout << "  Timestamp: skipped (first column)\n";
        ...
        std::vector<size_t> valid_counts(num_features, 0);
        ...
        size_t constant_features = 0, empty_features = 0, high_missing = 0;
        for (size_t i = 0; i < num_features; ++i) {
            if (valid_counts[i] == 0) {
                empty_features++;
            } else if (std::abs(max_vals[i] - min_vals[i]) < 1e-6f) {
                constant_features++;
            }
            if (nan_counts[i] > data.size() / 10) high_missing++;
        }

        std::cout << "  Constant features: " << constant_features << "\n";
        std::cout << "  Empty features: " << empty_features << "\n";
        std::cout << "  Variable features: "
                  << (num_features - constant_features - empty_features) << "\n";
        if (high_missing > 0) {
            std::cout << "  WARNING: " << high_missing
                      << " features have >10% missing values\n";
        }
        ...
        if (!header.empty() && config.has_header) {
            ...
            for (size_t i = 0; i < std::min(size_t(5), num_features); ++i) {
                if (start + i < header.size() && valid_counts[i] > 0) {
                    std::cout << "  [" << i << "] " << header[start + i]
                              << ": [" << std::fixed << std::setprecision(3)
                              << min_vals[i] << ", " << max_vals[i] << "]"
                              << " (" << nan_counts[i] << " missing)\n";
                }
            }
        }
    }
