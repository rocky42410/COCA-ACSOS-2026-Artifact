...
struct COCAConfig {
    ...
    // control whether FeatureProcessor downweights reconstruction
    // loss for features that were constant during training.
    //   true  - original behavior, weight = constant_weight on constant features.
    //   false - no downweighting (recommended default for anomaly detection).
    //           Features quiescent during "normal" become highly informative
    //           under the anomaly (e.g. foot forces at rest, then a kick moves
    //           a leg off the ground). Downweighting those would mask the
    //           very signal we want to detect.
    bool downweight_constant_features = false;
    float constant_weight = 0.1f;
};
...
public:
    void compute_stats(const std::vector<std::vector<float>>& windows, 
        ...
        D = feature_dim;
        min_std = min_std_val;
        feature_mean.assign(D, 0.0f);
        feature_std.assign(D, 0.0f);
        is_constant.assign(D, false);
        constant_indices.clear();

        // per-feature valid-sample counts. NaN samples are skipped
        // (consistent with csv_reader's NaN-on-missing behavior), and mean/std
        // are divided by the count of valid samples for each feature
        // individually. The previous code divided by total_samples (windows*T)
        // even when NaN was skipped, systematically underestimating stdev for
        // any feature with sparse coverage.
        std::vector<size_t> valid_count(D, 0);

        // First pass: sum values per feature (skip NaN/Inf)
        ...
        for (size_t d = 0; d < D; ++d) {
            if (valid_count[d] > 0) {
                feature_mean[d] /= valid_count[d];
            } else {
                feature_mean[d] = 0.0f;  // fully-missing feature; normalize will zero it
            }
        }
        ...
        // Compute std with minimum threshold (Bessel-corrected)
        for (size_t d = 0; d < D; ++d) {
            if (valid_count[d] > 1) {
                feature_std[d] = std::sqrt(feature_std[d] / (valid_count[d] - 1));
            } else {
                feature_std[d] = min_std;
            }
        ...
        // warn on fully-missing features. These contribute nothing
        // to training, but still inflate D and add dead dimensions to prompts.
        size_t empty = 0;
        for (size_t d = 0; d < D; ++d) if (valid_count[d] == 0) empty++;
        if (empty > 0) {
            std::cout << "  WARNING: " << empty
                      << " features had no valid samples. Consider removing from config.\n";
        }
    }
    
    // when csv_reader produces NaN for missing cells, normalize
    // substitutes 0 (the standardized mean) before the value reaches the
    // network. This preserves the original forward-pass behavior - the
    // encoder never sees NaN and cannot propagate it - while compute_stats
    // (above) excludes NaN from mean/std
    // computation, so the "missing" cells aren't biasing feature statistics.
    // High-missingness features are warned about at stats time so the user
    // can drop them from the config if they aren't informative.
    std::vector<float> normalize(const std::vector<float>& window, size_t T) const {
        ...
        for (size_t t = 0; t < T; ++t) {
            for (size_t d = 0; d < D; ++d) {
                ...
                if (std::isnan(val) || std::isinf(val)) {
                    // Substitute the standardized mean (zero after
                    // centering). Does not bias stats because compute_stats
                    // excluded this sample from the mean/std calculation.
                    normalized[idx] = 0.0f;
                    continue;
                }
...
public:
    ...
    void compute_losses(const std::vector<float>& x, ForwardResult& fwd) {
        ...
        // reconstruction loss with opt-in constant-feature weighting.
        // Previous behavior unconditionally downweighted reconstruction for
        // features flagged as "constant" during training (stdev < min_std).
        // For anomaly detection this was backward: features that are still
        // during normal operation are exactly the ones that move under the
        // anomaly (foot forces at rest vs during a kick; motor torques at
        // standing gravity-comp vs under external disturbance). Weighting
        // them at 0.1 masks the signal we want to detect.
        //
        // Default (downweight_constant_features = false): all features
        // weighted equally. Set true in the YAML config to restore original
        // behavior, or tune config.constant_weight separately.
        fwd.rec_loss = 0.0f;
        ...
        for (size_t b = 0; b < batch_size; ++b) {
            for (size_t i = 0; i < config.T * config.D; ++i) {
                ...
                float weight = 1.0f;
                if (config.downweight_constant_features &&
                    !constant_mask.empty() &&
                    (i % config.D) < constant_mask.size()) {
                    size_t feature_idx = i % config.D;
                    if (constant_mask[feature_idx]) weight = config.constant_weight;
                }
