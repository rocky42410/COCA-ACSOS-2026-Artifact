...
public:
    static bool load_config(const std::string& filename, COCAConfig& config) {
        ...
        // constant-feature downweighting (opt-in).
        // Default false. Set to true to enable the behavior where
        // features with stdev < min_std have their reconstruction loss
        // downweighted by constant_weight (default 0.1).
        if (params.count("downweight_constant_features")) {
            std::string v = params["downweight_constant_features"];
            config.downweight_constant_features = (v == "true" || v == "1" || v == "yes");
        }
        if (params.count("constant_weight")) config.constant_weight = std::stof(params["constant_weight"]);
    ...
    static void save_config(const std::string& filename, const COCAConfig& config) {
        ...
        // constant-feature downweighting (opt-in)
        file << "# Constant feature handling\n";
        file << "downweight_constant_features: "
             << (config.downweight_constant_features ? "true" : "false") << "\n";
        file << "constant_weight: " << config.constant_weight << "\n";
    }
